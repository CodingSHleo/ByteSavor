...
async def register(req: RegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    if req.username.strip():
        ...
        token = create_token(user.id, user.username or user.openid or "", role=(user.role or "user"))
    ...
    if exist:
        token = create_token(exist.id, exist.openid, role=(exist.role or "user"))
    ...
    token = create_token(user.id, user.openid, role=(user.role or "user"))
...
@router.post("/v1/auth/admin-register", tags=["Auth"])
async def admin_register(req: RegisterRequest, db: AsyncSession = Depends(get_db)):
    """管理员注册（仅用于初始化管理员账号）。"""
    await user_svc.ensure_user_auth_columns(db)
    username = req.username.strip()
    if not username or not user_svc.validate_username_format(username):
        return ErrorResponse(error={"code": "INVALID_USERNAME", "message": "用户名需要 3-32 位字母、数字、下划线或短横线"})
    ok, err = user_svc.validate_password_strength(req.password)
    if not ok:
        return ErrorResponse(error={"code": "WEAK_PASSWORD", "message": err})
    exist = await user_svc.get_user_by_username(db, username)
    if exist:
        exist.role = "admin"
        await db.commit()
        token = create_token(exist.id, exist.username or exist.openid or "", role="admin")
        return SuccessResponse(data={"token": token, "user_id": exist.id, "name": exist.name, "username": exist.username, "is_new": False, "role": "admin"})
    user = await user_svc.create_password_user(db, username, req.password, name=req.name, role="admin")
    token = create_token(user.id, user.username or user.openid or "", role="admin")
    return SuccessResponse(data={"token": token, "user_id": user.id, "name": user.name, "username": user.username, "is_new": True, "role": "admin"})
